...
import "./katalogSpiskom.css"
import ReactPaginate from "react-paginate";
import { useDispatch, useSelector } from "react-redux";
import CardSkeleton from "../../glavnaya/CardSkeleton";
import Modal from "../../glavnaya/module";
import { decrement, increment, setLength, setValue } from "../../../store/actions/zakladkiAction";
import AwesomeSlider from 'react-awesome-slider';
import 'react-awesome-slider/dist/styles.css';

export default function katalogSpiskom(link){
  const dispatch = useDispatch();
  const history=useNavigate();
  const [currentPage,setCurrentPage] = useState(null);
  const [modalActive,setModalActive] = useState(false);
  const [KatalogPerPage]= useState(3);
    ...
    useEffect(()=>{
      if (login){
        axios.get(`/users?login=${login}`).then((data) => {
          if (data.data[0].zakladki.length>0){
            dispatch(setLength(data.data[0].zakladki.length))
          }
        })
      }
    },[]);
    ...
    if (login){
      axios.get(`/users?login=${login}`).then((data)=>{
        if(heart.classList.value=="heart__svg active"){
          ...
          dispatch(decrement(lengthZakladki));
          notify3();
        }
        else{
          setData2(true);
          dispatch(increment(lengthZakladki));
          axios.patch(`/users/${id}`, zakladki.length?{
            "zakladki":
              [...zakladki,item]
          }:{
            "zakladki":
  ...
  const handlepageclick = (e) =>{
    const newOffset = (e.selected*KatalogPerPage)%link.children.length;
    setItemOffset(newOffset)
  }

  const push = (item,item2) =>{
    history(`/katalog/city=${item2}/${item}`)
  }
  useEffect(()=>{
    if(login){
      axios.get(`/users?login=${login}`).then((data2) => {
        for (let i=0; i<link.children.length;i++){
          for (let j=0;j<data2.data[0].zakladki.length;j++){
            if(link.children[i].id==data2.data[0].zakladki[j].id){
    ...
    setLoading(false);
  },[data2,login,itemOffset,KatalogPerPage])

  useEffect(()=>{
    const endoffset=itemOffset+KatalogPerPage;
    setCurrentPage(link.children.slice(itemOffset,endoffset));
    setCountPage(Math.ceil(link.children.length/KatalogPerPage));
  },[itemOffset,KatalogPerPage,link])

  useEffect(() => {
    if(login){
      axios.get(`/users?login=${login}`).then((data) => {
        for (let i = 0; i < data.data[0].zakladki.length; i++) {
          if(data.data[0].zakladki[i].id==value){
            data.data[0].zakladki.splice(i,1);
            axios.patch(`/users/${id}`,{
              "zakladki":data.data[0].zakladki
            })
          }
        }
        setZakladki(data.data[0].zakladki);
        setData2(false);
      });
    }
  },[data2,login,value]);
              ...
              <Col key={item.id} style={{marginBottom:"25px"}}>
                <Card className="card__style" style={{width:"100%",height:"300px"}}>
                  <div className="SpisokInformKontakti">  
                    <div className="CardOsnova" style={{display: "flex"}}>
                      ...
                      <div className="">
                        <AwesomeSlider style={{height:"300px",width:"443px"}}> 
                          <Card.Img  className="imgCard" style={{ height: "300px",width: "444px"}} data-src={item.url} src={item.url}/>
                        </AwesomeSlider>   
                      </div>
                      <Card.Body className="bodyCard" >
                        <Card.Title className="card__title">
                          <div className="perviytitle">
                            <div className="TitleSpiskom">
                              <h1 className="TitleTextInnerKatalogSpiskom">{item.komnati} апартаменты на {item.metro_title}</h1>
                            </div>
                            <div className="stoimostobshaya">
                              <div className="stoimost">{(Math.round(item.sent * 100) / 100).toFixed(2)} BYN</div>
                          ...
                          <div className="adresall" style={{marginBottom:"20px"}}>
                            ...
                            <div className="itemsCategories" style={{alignItems:"center"}}>
                              <div className="kolvoludey" style={{marginRight:"15px"}}>
                              ...
                              <div className="kolvoludey komnati" style={{marginRight:"15px"}}><span className="kolvoKomtan">{item.komnati}</span></div>
                              <div className="kolvoludey Ploshad" style={{marginRight:"15px"}}>{item.ploshad}<sup style={{lineHeight:"5px"}}><small>2</small></sup></div>
                              <div className="metrorayon" style={{marginTop:0,alignItems:"center"}}>
                              <div className="kolvoludey metroall" style={{marginRight:"15px"}}>
                              ...
                              <div className="kolvoludey rayonall" style={{marginRight:"15px"}}>
                        ...
                        <Card.Text className="card__text card__text2">
                          ...
                          <div className="btnkontaktiOsnova" style={{border:"none",padding:0,marginTop:"24px",width:"63.15%"}}>
                            <div className="btnKontaktiHeart">
                              <div className="dropdownKontakti">
                                <Button variant="primary" className="kontaktibtn kontaktibtn3" id={item.id} onClick={()=>btnclick(item.id)}>
                                ...
                                <div className="informkontakti" style={{marginTop:"-9%",left:"-13%"}} id={item.id}>
                            ...
                            <Button variant="primary" className="Podrobneebtn Podrobneebtn3">
                            <div className="Podrobnee">
                                <p className="textPodrobnee" onClick={(e)=>push(item.id,item.city)}>Подробнее</p>
                            </div>
                            </Button>
                        </div>
                      </Card.Body>
            ...
            <div className="" style={{width:"100%",display:"flex",justifyContent:"space-between"}}>
             <ReactPaginate
              ...
              <div className="Givemeall">
                ...
                <svg className="iconsockatalogselect" width="36" height="35" viewBox="0 0 36 35" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <g clip-path="url(#clip0_2831_894)">
                  <rect x="0.968262" y="0.427734" width="34.1461" height="34.1461" rx="17.073" fill="#664EF9" fill-opacity="0.1"/>
                  <g clip-path="url(#clip1_2831_894)">
                  <path className="iconsockatalogselect__hover" d="M20.8839 11.7977H22.4426V9.08311C22.1737 9.04612 21.2489 8.96289 20.1718 8.96289C17.9246 8.96289 16.3852 10.3764 16.3852 12.9743V15.3653H13.9053V18.4H16.3852V26.0359H19.4256V18.4007H21.8052L22.1829 15.366H19.4249V13.2753C19.4256 12.3981 19.6618 11.7977 20.8839 11.7977Z" fill="#000000"/>
                  </g>
                  </g>
                  <defs>
                ...
                <svg className="iconsockatalogselect" width="36" height="35" viewBox="0 0 36 35" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <g clip-path="url(#clip0_2831_898)">
                  <rect x="0.902405" y="0.427734" width="34.1461" height="34.1461" rx="17.073" fill="#664EF9" fill-opacity="0.1"/>
                  <path className="iconsockatalogselect__hover" d="M25.2089 10.9974L10.5659 16.644C9.56662 17.0454 9.5724 17.6029 10.3826 17.8515L14.142 19.0242L22.8403 13.5362C23.2516 13.2859 23.6274 13.4206 23.3185 13.6948L16.2712 20.0549H16.2695L16.2712 20.0557L16.0119 23.9308C16.3918 23.9308 16.5594 23.7565 16.7725 23.5509L18.5985 21.7752L22.3968 24.5808C23.0971 24.9665 23.6001 24.7683 23.7744 23.9325L26.2677 12.1817C26.5229 11.1585 25.8771 10.6951 25.2089 10.9974Z" fill="#000000"/>
                  </g>
                  <defs>
