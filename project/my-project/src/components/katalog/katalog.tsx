...
import KatalogSpiskom from "./spiskom/katalogSpiskom";
import KatalogOsnovnoy from "./osnovnoy/katalogOsnovnoy";
import {useLocation,useNavigate,Link, NavLink} from "react-router-dom";

import axios from "../../axios";
import "./osnovnoy/pagination.css";
import click from "../glavnaya/Homepagejs";
import ClickCheckbox from "../glavnaya/ClickCheckBox";
import categoriaClick from "./osnovnoy/CategoriaClick";
import PokazatSort from "./osnovnoy/Filter";

import Clear from "./osnovnoy/ClearBtn";
import { cityIn, cityFrom, cityTo } from 'lvovich';
import { Row } from "react-bootstrap"
import "./katalog.css"
import { useDispatch, useSelector } from "react-redux";
import { clearFilter,setCity,setFilterAll,setKvartiri,setSort } from "../../store/slices/FilterSlice";
import qs from "qs";
...
export default function katalog(){
  ...
  const [data2,setData2]=useState<any>(true);
  ...
  const Show = (value,value2,filterKomnati,filterMesta,filterRayon,filterMetro,input) => {
    const item=[filterRayon,filterKomnati,value,value2,filterMesta,filterMetro,input,gorod]; 
    ...
    PokazatSort(value,value2,category,setCategory,setValue,setValue2,axios,setArenda,Arenda,gorod);
    navigate(`/katalog/city=?${queryString}`)
  }

  // Кнопка Очистить
  const ClearAll = (category,setCategory,setValue,setValue2,axios,setArenda,gorod) => {
    dispatch(clearFilter(filterFromHome));
    Clear(category,setCategory,setValue,setValue2,axios,setArenda,gorod);
    navigate("/katalog/city=");
  }

  // Значениея только численные в price
  const [value, setValue] = useState(filter.priceMin!==0?filter.priceMin:"");
  const handleChange = event => {
    const result = event.target.value.replace(/\D/g, '');
    setValue(result);
  };

  const [value2, setValue2] = useState(filter.priceMax!==0?filter.priceMax:"");
  ...
  useEffect(()=>{
    if (window.location.search) {
      ...
      const item=[params.Rayon,params.rooms,params.priceMin,params.priceMax,params.sleepPlaces,params.metro,params.inputCheckboxInfo,params.gorod]; 
      dispatch(setFilterAll(item));
      dispatch(setKvartiri(""));
      dispatch(setCity(params.gorod));
      const queryString = (qs.stringify({
        gorod: params.gorod,
        rooms:params.rooms,
        ...
        Rayon:params.Rayon,
        metro:params.metro,
        checkboxInfo:params.inputCheckboxInfo
      }))
      navigate(`/katalog/city=?${queryString}`) 
      if (category=="" && sort=="По умолчанию"){
      ...
      else if (category2=="komnati"){
        console.log(123)
        axios.get(`/ArendaCard?city2=${gorod}&komnati=${category?category.substring(0,6):""}.`)
        .then(({data})=>{
          setArenda(data);
        })
      }
        ...
        if(sort=="По комнатам"){
          console.log("4(1)");
          newItem =(Arenda.sort((a,b)=> a.komnati.substring(0,1) - b.komnati.substring(0,1)))
          setArenda(newItem)
        }
          ...
          console.log("4(3)")
          newItem = Arenda.sort((a,b)=> a.ploshad.substring(0,a.ploshad.indexOf(" ",0)) - b.ploshad.substring(0,b.ploshad.indexOf(" ",0)));
          setArenda(newItem)
        }
      }
        ...
        console.log(6);
        axios.get(`/ArendaCard?city2=${gorod}&rayon=${category}`).then(({data})=>{
          setArenda(data);
      })  
      setData2(!data2)
      }
  ...
  return(
        ...
        <div className="conteiner">
          <div className="hlebnie">
            <nav className="breadcrumbs" style={{display:"flex"}}>
              ...
              <Link to={`/katalog/city=`} style={{textDecoration:"none"}}>
                <div className="katalogLink">
                  <p className="LinkText">Квартиры в {cityIn(gorod?gorod:"Минск")}</p>
                </div>
              </Link>
            </nav>
          </div>
            ...
            <div className="textFilterInfo">
              <div className="ArendaInnerText">
                <h1 className="ArendaInnerTextH1">
                    Аренда квартир на сутки в {cityIn(gorod?gorod:"Минск")}
                </h1>
              </div>
              ...
              <div className="categories" style={{justifyContent:"flex-start"}}>
                {
                  categories.map((item)=>{
                    return(
                      <div className="categoriesBtn" id={item.id} onClick={()=>{categoriaClick(item.id,item.categoria,category,setCategory,setCategory2,setValue,setValue2,gorod);navigate("/katalog/city=")}}>
    ...
    <section className="secondKatalog">
      <div className="SecondKatalogOsnova">
        <div className="conteiner">
          <div className="allitemsselectkatalog">
            <div className=" select__filter__item3">
              <p className="select__filter__item__css select__filter__item__css3">Комнаты</p>
                <div className="dropdown">
                  <button className="Spisok" id="SpisokFilter" onClick={click}>{filter.rooms!=="Выберите"?filter.rooms:"Выберите"}</button>
            ...
            <div className=" select__filter__item3">
              ...
              <div className="filter">
                <input  className="filter__input" id="filter__input1" type="text" placeholder="От" value={filter.priceMin?filter.priceMin:value} onChange={handleChange}/>
                <p className="filter__text">-</p>
                <input className="filter__input" id="filter__input2" type="text" placeholder="До" value={filter.priceMax?filter.priceMax:value2} onChange={handleChange2}/>
              </div>
            </div>
            <svg width="2" height="81" viewBox="0 0 2 81" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path opacity="0.1" d="M1 0L1 81" stroke="#664EF9"/>
            ...
            <div className=" select__filter__item2">
              <button className="clearAll" onClick={()=>{ClearAll(category,setCategory,setValue,setValue2,axios,setArenda,gorod)}}>Очистить</button>
            </div>
            <div className=" 2">
              <button className="Pokazat" onClick={()=>Show(value,value2,filterKomnati.textContent,filterMesta.textContent,filterRayon.textContent,filterMetro.textContent,(input as HTMLInputElement).value)}>
      ...
      <div className="DopOpcii" style={{display:"flex"}}>
        <div className="OsnovaOpcii" style={{maxWidth:"1308px"}}>
          {/* Первое деление */}
          <div className="delenieOpcii">
            <p className="titleOpcii">Спальные места</p>
            <div className="dropdown">
              <button className="Spisok" id="FilterMesta" onClick={click}>{filter.sleepPlaces!=="Выберите"?filter.sleepPlaces:"Выберите"}</button>
              <ul className="Spisok__dropdown">
            ...
            <input type="text" id="checkboxInputValue" name="select__category" value={filter.inputCheckboxInfo!==""?filter.inputCheckboxInfo:""} className="drodown__item__hiden" />
        ...
        {/* Второе деление */}
          <div className="delenieOpcii">
            ...
            <div className="dropdown">
              <button className="Spisok" id="FilterRayon" onClick={click}>{filter.rayon!=="Выберите"?filter.rayon:"Выберите"}</button>
              <ul className="Spisok__dropdown">     
                <>
                {
                    Rayon.map((item)=>{
          ...
          <div className="delenieOpcii">
            ...
            <div className="dropdown">
              <button className="Spisok" id="FilterMetro2" onClick={click}>{filter.metro!=="Выберите"?filter.metro:"Выберите"}</button>
              <ul className="Spisok__dropdown">
    ...
    <section className="thirdKatalog">
      <div className="thirdOsnovnoe">
        <div className="conteiner">
          <div className="delenieThirdKatalog">
            ...
            <svg style={{marginLeft:"10px",marginTop:"9px",position:"absolute"}} width="15" height="16" viewBox="0 0 15 16" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <button className="Spisok Spisok2" id = "SortirovkaBtn" onClick={sortirovka}>
                {sort}
              </button>
              <ul className="Spisok__dropdown" id="Spisok__dropdownSortirovka">
            ...
            <div className="secondDelenieThirdblockKatalog">
              <button className={ToggleState === 1 ? "spisok__active":"spisok"} onClick={()=>{toggleTab(1)}}>
                <div style={{display:"flex", alignItems:"center"}}>
                  <svg style={{marginRight:"10px"}}width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <rect className="changeFillKatalog" y="0.5" width="14.5036" height="3.22302" rx="1" fill="#7A7F86"/>
                    <rect className="changeFillKatalog" y="11.2773" width="14.5036" height="3.22302" rx="1" fill="#7A7F86"/>
                    <rect className="changeFillKatalog" y="5.88867" width="14.5036" height="3.22302" rx="1" fill="#7A7F86"/>
                  </svg>
                  <p>Список</p> 
                </div>
              </button>
              <button className={ToggleState === 2 ? "Plitki__active":"Plitki"} onClick={()=>{toggleTab(2)}}>
                <div style={{display:"flex", alignItems:"center"}}>
                  <svg style={{marginRight:"10px"}} width="15" height="14" viewBox="0 0 15 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <rect className="changeFillKatalogPlitki" x="0.00390625" width="5.44444" height="5.44444" rx="1" fill="#664EF9"/>
                    <rect className="changeFillKatalogPlitki" x="0.00390625" y="8.55469" width="5.44444" height="5.44444" rx="1" fill="#664EF9"/>
                    <rect className="changeFillKatalogPlitki" x="8.55957" width="5.44444" height="5.44444" rx="1" fill="#664EF9"/>
                    <rect className="changeFillKatalogPlitki" x="8.55957" y="8.55469" width="5.44444" height="5.44444" rx="1" fill="#664EF9"/>
                  </svg>
                  <p>Плитки</p> 
                </div>
              </button>
              <NavLink to = "/Map">
                <button className="NaKarte">
                  <div style={{display:"flex", alignItems:"center"}}>
                    ...
                    <p className="PokazatMap">Показать на карте</p>
                  </div>
                </button>
              </NavLink>
            </div>
    ...
    <section className="CardsKatalog">
        ...
        <div className="conteiner">
          <div className="SetkaCards">
            ...
            <Row style={{display:"flex", flexWrap:"wrap",justifyContent:"space-between"}}>
            {
              Arenda.length?
                (ToggleState==2)?(  
                  <KatalogOsnovnoy>{Arenda}</KatalogOsnovnoy>
                  ):<KatalogSpiskom>{Arenda}</KatalogSpiskom>
    ...
    <div className="Forma formaKatalog">
      <div className="conteiner">
