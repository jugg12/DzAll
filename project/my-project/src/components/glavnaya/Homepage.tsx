...
import { cityIn, cityTo,inclineLastname } from 'lvovich';
...
import ClickCheckbox from "../glavnaya/ClickCheckBox";
...
import {setCategoryId,setCategoryInfoId,setCity,setCityRayonHomePage,setFilterAll,clearFilter, setKvartiri, setSort} from "../../store/slices/FilterSlice";
...
export default function Homepage(){
  ...
  const [arendaLength,setArendaLength] =useState();
  const [arendaLength2,setArendaLength2] =useState();
  const [arendaLength3,setArendaLength3] =useState();
  const [arendaLength4,setArendaLength4] =useState();
  const [arendaLength5,setArendaLength5] =useState();
  const [arendaLength6,setArendaLength6] =useState();
  const [Cities,setCities]=useState([]);
  const [imgUrl,setImgUrl] = useState <any>();
  const [imgUrl2,setImgUrl2] = useState <any>();
  const [Rayon,setRayon] = useState<any>([]);
  const [obyavlenia,setObyavlenia] = useState<any>([]);
  ...
  localStorage.setItem("kvartiri","Квартиры на сутки");
  localStorage.setItem("value","0");
  localStorage.setItem("rayon","");
  ...
  const FilterMestaDop = document.getElementById("FilterMestaDop");
  // redux
  ...
  const Show = (value,value2,filterKomnati,filterMesta,filterRayon,filterMetro,input) =>{
    if (inputCity.textContent=="Выберите"){
      notify3()
    }
    else {
      dispatch(setKvartiri(""))
      dispatch(setCity(inputCity.textContent));
      const item=[filterRayon,filterKomnati,value,value2,filterMesta,filterMetro,input,inputCity.textContent]; 
      dispatch(setFilterAll(item));
      navigate("/katalog/city=?")
    }
  }
  ...
  const click2 = () => {
    document.querySelectorAll(".dropdown4").forEach(function(dropdownWrapper){
      ...
      const btnclick = dropdownWrapper.querySelector(".Spisok__dropdown__custom");
      
      btnclick.classList.toggle("Spisok__dropdown__custom__visible");
      listitemclick.forEach(function(listitem){
        listitem.addEventListener("click",(e)=>{ //Клик на элемент в листе
          btnclick.classList.remove("Spisok__dropdown__custom__visible");
  ...
  const validationSchema=yup.object().shape({
    ...
    komnati:yup.string().required(),
    kolvo:yup.string().required(),
    ploshad:yup.number().required("Неверная площадь"),
  ...
  useEffect(() => {
    ...
    axios.get(`/ArendaCard?city=Гомеле`).then(({data})=>{
      setArendaLength2(data.length);
      setArenda(data);
    })
    axios.get(`/ArendaCard?city=Гродно`).then(({data})=>{
      setArendaLength3(data.length);
      setArenda(data);
    })
    axios.get(`/ArendaCard?city=Могилеве`).then(({data})=>{
      setArendaLength4(data.length);
      setArenda(data);
    })
    axios.get(`/ArendaCard?city=Бресте`).then(({data})=>{
      setArendaLength5(data.length);
      setArenda(data);
    })
    axios.get(`/ArendaCard?city=Витебске`).then(({data})=>{
    ...
    dispatch(setKvartiri("Квартиры на сутки"));
    ...
    document.querySelectorAll(".dropdown").forEach(function(dropdownWrapper){
      ...
      listitemclick.forEach(function(listitem){
        listitem.addEventListener("click",(e)=>{
          ...
          (input as HTMLInputElement).value=btn.textContent;
          if (input3!==null){
            if ((input3 as HTMLInputElement).value=="")
            {
              dopinfo__pervogo__delenia.textContent = `Аренда квартир в ${cityIn((input2 as HTMLInputElement).value)}`;
              dopkol_vo__predlojeniy.textContent=`Предложений в ${cityIn((input2 as HTMLInputElement).value)}`;
              localStorage.setItem("city2",(input2 as HTMLInputElement).value);
              const item = [(input2 as HTMLInputElement).value,"Шабаны р."]
              ...
              const item = [(input2 as HTMLInputElement).value,(input3 as HTMLInputElement).value];
              dopinfo__pervogo__delenia.textContent = `Аренда квартир в ${cityIn((input2 as HTMLInputElement) .value)} в ${cityIn((input3 as HTMLInputElement).value)} районе`;
              dopkol_vo__predlojeniy.textContent=`Предложений в ${cityIn((input2 as HTMLInputElement).value)}`;
              dispatch(setCityRayonHomePage(item))
              const queryString = (qs.stringify({
                cityFilter:item[0],
  ...
  const OpciiClick = () => {
    opcii.classList.toggle("active");
    dopopcii.classList.toggle("active");
  }

  useEffect(()=>{
    if (login){
      axios.get(`/users?login=${login}`).then((data) => {
        setObyavlenia(data.data[0].obyavlenia);
        setData2(false) 
      });
    }
  },[data2,login])
  ...
  const addArenda = (value) => {
    ...
    if(vibor==true){
          const item = {
            "id": obyavlenia.length+1,
            "city" : value.city,  
            "sent" : value.sent,
            "komnati":value.komnati,
            "ploshad":value.ploshad,
            "metro":value.metro,
            "rayon":value.rayon,
            "opisanie":value.opisanie,
            "url":imgUrl,
            "imageVladelec":imgUrl2,
            "name":value.name,
            "number":value.number,
            "mail":value.mail,
            ...
            "opcii":[
              {
                "name":((checkboxInputValue_module as HTMLInputElement).value!=="")?
                      ...
                      :[],
                "mesta":FilterMestaDop.textContent!=="Выберите"? FilterMestaDop.textContent:""
              }
            ],
            "kolvo":value.kolvo,
            "proverka":"На рассмотрении",
          }
          
          axios.patch(`/users/${id}`, obyavlenia.length?{
            "obyavlenia":
              [...obyavlenia,item]
          }:{
            "obyavlenia":
            ...
            FilterMestaDop.textContent="Выберите";
            value.kolvo="";
            ...
            <div className="block__item">
              ...
              <div className="all" >
                <div className="block__tabs">
                    ...
                    <div className={filter.category === 2 ? "tabs tabs__active":"tabs"} onClick={()=>{dispatch(setCategoryId(2));  if(opcii!==null){opcii.classList.remove("active");dopopcii.classList.remove("active")}}}>
                      <p className="select__item" >Котеджи и усадьбы</p>
                    </div>
                    <div className={filter.category === 3 ? "tabs tabs__active":"tabs"} onClick={()=>{dispatch(setCategoryId(3));  if(opcii!==null){opcii.classList.remove("active");dopopcii.classList.remove("active")}}}>
                      <p className="select__item">Бани и сауны</p>
                    </div>
                    <div className={filter.category === 4 ? "tabs tabs__active":"tabs"} onClick={()=>{dispatch(setCategoryId(4));  if(opcii!==null){opcii.classList.remove("active");dopopcii.classList.remove("active")}}}>
              ...
              <div className="select__filter">
                <div className={filter.category===1? "select__filter__item":"select__filter__item__hidden"}>
                  <p className="select__filter__item__css">Город</p>
                    <div className="dropdown">
                      <button className="Spisok" id="#inputCity" onClick={click} >Выберите</button >
                ...
                <div className={filter.category===1? "select__filter__item":"select__filter__item__hidden"}>
                  <p className="select__filter__item__css">Комнаты</p>
                    <div className="dropdown">
                      <button className="Spisok" id="SpisokFilter" onClick={click}>Выберите</button>
                      <ul className="Spisok__dropdown">
                ...
                <div className="filter">
                  <input  className="filter__input" id="filter__input1" type="text" placeholder="От" value={value} onChange={handleChange}/>
                  <p className="filter__text">-</p>
                  <input className="filter__input" id="filter__input2" type="text" placeholder="До" value={value2} onChange={handleChange2}/>
                </div>
                </div>
                <div className={filter.category===1? "select__filter__item":"select__filter__item__hidden"}>
                  <div className="raz Opcii" onClick={OpciiClick}>
                ...
                <div className={filter.category===1? "select__filter__item select__filter__item4":"select__filter__item__hidden"}>
                      <button className="Voyti choose" onClick={()=>Show(value,value2,filterKomnati.textContent,filterMesta.textContent,filterRayon.textContent,filterMetro.textContent,(input as HTMLInputElement).value)}>
                      ...
                      <button className="Voyti choose choose2" onClick={()=>Show(value,value2,filterKomnati.textContent,filterMesta.textContent,filterRayon.textContent,filterMetro.textContent,(input as HTMLInputElement).value)}>
                        <div className="btnarrow">
                          <svg width="7" height="12" viewBox="0 0 7 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                          <path d="M1.21484 10.75L5.96484 6L1.21484 1.25" stroke="#242424" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                          </svg>
              ...
              <div className="DopOpcii">
                <div className="OsnovaOpcii">
                  {/* Первое деление */}
                  <div className="delenieOpcii">
                    <p className="titleOpcii">Спальные места</p>
                    <div className="dropdown">
                      <button className="Spisok" id = "FilterMesta" onClick={click}>Выберите</button>
                      <ul className="Spisok__dropdown">
                    ...
                    <input type="text" id="checkboxInputValue" className="drodown__item__hiden" />
                    <div className="checkbox" style={{display:"flex",marginBottom:"10px",marginTop:"30px"}}>
                      <input type="checkbox" value={"Газовая плита"} id="checkbox1" onChange={() => ClickCheckbox(1)} className="checkBoxOpcii"/> 
                      <label htmlFor="checkbox1" className="textCheckboxOpcii">Газовая плита</label>
                    </div>
                    <div className="" style={{display:"flex",marginBottom:"10px"}}>
                      <input type="checkbox" value={"Духовка"} id="checkbox2" onChange={() => ClickCheckbox(2)} className="checkBoxOpcii"/> 
                      <label htmlFor="checkbox2" className="textCheckboxOpcii">Духовка</label>
                    </div>
                    <div className="" style={{display:"flex",marginBottom:"10px"}}>
                      <input type="checkbox" value={"Кофеварка"} id="checkbox3" onChange={() => ClickCheckbox(3)} className="checkBoxOpcii"/> 
                      <label htmlFor="checkbox3" className="textCheckboxOpcii">Кофеварка</label>
                    </div>
                    <div className="" style={{display:"flex",marginBottom:"10px"}}>
                      <input type="checkbox" value={"Микроволновая печь"} id="checkbox4" onChange={() => ClickCheckbox(4)} className="checkBoxOpcii"/> 
                      <label htmlFor="checkbox4" className="textCheckboxOpcii">Микроволновая печь </label>
                    </div>
                    <div className="" style={{display:"flex",marginBottom:"10px"}}>
                      <input type="checkbox" value={"Посуда"} id="checkbox5" onChange={() => ClickCheckbox(5)} className="checkBoxOpcii"/> 
                      <label htmlFor="checkbox5" className="textCheckboxOpcii">Посуда </label>
                    </div>
                    <div className="" style={{display:"flex",marginBottom:"10px"}}>
                      <input type="checkbox" value={"Посудомоечная машина"} id="checkbox6" onChange={() => ClickCheckbox(6)} className="checkBoxOpcii"/> 
                      <label htmlFor="checkbox6" className="textCheckboxOpcii">Посудомоечная машина </label>
                    </div>
                  </div>
                {/* Второе деление */}
                  <div className="delenieOpcii">
                    <p className="titleOpcii">Район</p>
                    <div className="dropdown">
                      <button className="Spisok" id="FilterRayon" onClick={click}>Выберите</button>
                      <ul className="Spisok__dropdown">     
                        <>
                        {
                            Rayon.map((item)=>{
                    ...
                    <div className="checkbox" style={{display:"flex",marginBottom:"10px",marginTop:"30px"}}>
                      <input type="checkbox" value={"Газовая плита"} id="checkbox7" onChange={() => ClickCheckbox(7)} className="checkBoxOpcii"/> 
                      <label htmlFor="checkbox7" className="textCheckboxOpcii">Газовая плита</label>
                    </div>
                    <div className="" style={{display:"flex",marginBottom:"10px"}}>
                      <input type="checkbox" value={"Духовка"} id="checkbox8" onChange={() => ClickCheckbox(8)} className="checkBoxOpcii"/> 
                      <label htmlFor="checkbox8" className="textCheckboxOpcii">Духовка</label>
                    </div>
                    <div className="" style={{display:"flex",marginBottom:"10px"}}>
                      <input type="checkbox" value={"Кофеварка"} id="checkbox9" onChange={() => ClickCheckbox(9)} className="checkBoxOpcii"/> 
                      <label htmlFor="checkbox9" className="textCheckboxOpcii">Кофеварка</label>
                    </div>
                    <div className="" style={{display:"flex",marginBottom:"10px"}}>
                      <input type="checkbox" value={"Микроволновая печь"} id="checkbox10" onChange={() => ClickCheckbox(10)} className="checkBoxOpcii"/> 
                      <label htmlFor="checkbox10" className="textCheckboxOpcii">Микроволновая печь </label>
                    </div>
                    <div className="" style={{display:"flex",marginBottom:"10px"}}>
                      <input type="checkbox" value={"Посуда"} id="checkbox11" onChange={() => ClickCheckbox(11)} className="checkBoxOpcii"/> 
                      <label htmlFor="checkbox11" className="textCheckboxOpcii">Посуда </label>
                    </div>
                    <div className="" style={{display:"flex",marginBottom:"10px"}}>
                      <input type="checkbox" value={"Посудомоечная машина"} id="checkbox12" onChange={() => ClickCheckbox(12)} className="checkBoxOpcii"/> 
                      <label htmlFor="checkbox12" className="textCheckboxOpcii">Посудомоечная машина </label>
                    </div>
                  </div>
                  {/* Третие делене */}
                  <div className="delenieOpcii">
                    <p className="titleOpcii">Метро</p>
                    <div className="dropdown">
                      <button className="Spisok" id = "FilterMetro2" onClick={click}>Выберите</button>
                      <ul className="Spisok__dropdown">
                    ...
                    <div className="checkbox" style={{display:"flex",marginBottom:"10px",marginTop:"30px"}}>
                      <input type="checkbox" value={"Газовая плита"} id="checkbox13" onChange={() => ClickCheckbox(13)} className="checkBoxOpcii"/> 
                      <label htmlFor="checkbox13" className="textCheckboxOpcii">Газовая плита</label>
                    </div>
                    <div className="" style={{display:"flex",marginBottom:"10px"}}>
                      <input type="checkbox" value={"Духовка"} id="checkbox14" onChange={() => ClickCheckbox(14)} className="checkBoxOpcii"/> 
                      <label htmlFor="checkbox14" className="textCheckboxOpcii">Духовка</label>
                    </div>
                    <div className="" style={{display:"flex",marginBottom:"10px"}}>
                      <input type="checkbox" value={"Кофеварка"} id="checkbox15" onChange={() => ClickCheckbox(15)} className="checkBoxOpcii"/> 
                      <label htmlFor="checkbox15" className="textCheckboxOpcii">Кофеварка</label>
                    </div>
                    <div className="" style={{display:"flex",marginBottom:"10px"}}>
                      <input type="checkbox" value={"Микроволновая печь"} id="checkbox16" onChange={() => ClickCheckbox(16)} className="checkBoxOpcii"/> 
                      <label htmlFor="checkbox16" className="textCheckboxOpcii">Микроволновая печь </label>
                    </div>
                    <div className="" style={{display:"flex",marginBottom:"10px"}}>
                      <input type="checkbox" value={"Посуда"} id="checkbox17" onChange={() => ClickCheckbox(17)} className="checkBoxOpcii"/> 
                      <label htmlFor="checkbox17" className="textCheckboxOpcii">Посуда </label>
                    </div>
                    <div className="" style={{display:"flex",marginBottom:"10px"}}>
                      <input type="checkbox" value={"Посудомоечная машина"} id="checkbox18" onChange={() => ClickCheckbox(18)} className="checkBoxOpcii"/> 
                      <label htmlFor="checkbox18" className="textCheckboxOpcii">Посудомоечная машина </label>
                    </div>
                  </div>
                {/* Четвертое деление */}
                  <div className="delenieOpcii">
                  <div className="dropdown" style={{visibility: "hidden"}}>
                            <p className="titleOpcii">Метро</p>
                              <div className="dropdown">
                                <button className="Spisok" type="button">Выберите</button>
                                <input type="text" name="select__category"  className="drodown__item__hiden" />
                              </div>
                              </div>
                    <div className="checkbox" style={{display:"flex",marginBottom:"10px",marginTop:"30px"}}>
                        <input type="checkbox" value={"Газовая плита"} id="checkbox19" onChange={() => ClickCheckbox(19)} className="checkBoxOpcii"/> 
                        <label htmlFor="checkbox19" className="textCheckboxOpcii">Газовая плита</label>
                      </div>
                      <div className="" style={{display:"flex",marginBottom:"10px"}}>
                        <input type="checkbox" value={"Духовка"} id="checkbox20" onChange={() => ClickCheckbox(20)} className="checkBoxOpcii"/> 
                        <label htmlFor="checkbox20" className="textCheckboxOpcii">Духовка</label>
                      </div>
                      <div className="" style={{display:"flex",marginBottom:"10px"}}>
                        <input type="checkbox" value={"Кофеварка"} id="checkbox21" onChange={() => ClickCheckbox(21)} className="checkBoxOpcii"/> 
                        <label htmlFor="checkbox21" className="textCheckboxOpcii">Кофеварка</label>
                      </div>
                      <div className="" style={{display:"flex",marginBottom:"10px"}}>
                        <input type="checkbox" value={"Микроволновая печь"} id="checkbox22" onChange={() => ClickCheckbox(22)} className="checkBoxOpcii"/> 
                        <label htmlFor="checkbox22" className="textCheckboxOpcii">Микроволновая печь </label>
                      </div>
                      <div className="" style={{display:"flex",marginBottom:"10px"}}>
                        <input type="checkbox" value={"Посуда"} id="checkbox23" onChange={() => ClickCheckbox(23)} className="checkBoxOpcii"/> 
                        <label htmlFor="checkbox23" className="textCheckboxOpcii">Посуда </label>
                      </div>
                      <div className="" style={{display:"flex",marginBottom:"10px"}}>
                        <input type="checkbox" value={"Посудомоечная машина"} id="checkbox24" onChange={() => ClickCheckbox(24)} className="checkBoxOpcii"/> 
                        <label htmlFor="checkbox24" className="textCheckboxOpcii">Посудомоечная машина </label>
                      </div>
                  </div>
                {/* Пятое деление */}
                  <div className="delenieOpcii">
                  <div className="dropdown" style={{visibility: "hidden"}}>
                    ...
                    <div className="checkbox" style={{display:"flex",marginBottom:"10px",marginTop:"30px"}}>
                        <input type="checkbox" value={"Газовая плита"} id="checkbox25" onChange={() => ClickCheckbox(25)} className="checkBoxOpcii"/> 
                        <label htmlFor="checkbox25" className="textCheckboxOpcii">Газовая плита</label>
                      </div>
                      <div className="" style={{display:"flex",marginBottom:"10px"}}>
                        <input type="checkbox" value={"Духовка"} id="checkbox26" onChange={() => ClickCheckbox(26)} className="checkBoxOpcii"/> 
                        <label htmlFor="checkbox26" className="textCheckboxOpcii">Духовка</label>
                      </div>
                      <div className="" style={{display:"flex",marginBottom:"10px"}}>
                        <input type="checkbox" value={"Кофеварка"} id="checkbox27" onChange={() => ClickCheckbox(27)} className="checkBoxOpcii"/> 
                        <label htmlFor="checkbox27" className="textCheckboxOpcii">Кофеварка</label>
                      </div>
                      <div className="" style={{display:"flex",marginBottom:"10px"}}>
                        <input type="checkbox" value={"Микроволновая печь"} id="checkbox28" onChange={() => ClickCheckbox(28)} className="checkBoxOpcii"/> 
                        <label htmlFor="checkbox28" className="textCheckboxOpcii">Микроволновая печь </label>
                      </div>
                      <div className="" style={{display:"flex",marginBottom:"10px"}}>
                        <input type="checkbox" value={"Посуда"} id="checkbox29" onChange={() => ClickCheckbox(29)} className="checkBoxOpcii"/> 
                        <label htmlFor="checkbox29" className="textCheckboxOpcii">Посуда </label>
                      </div>
                      <div className="" style={{display:"flex",marginBottom:"10px"}}>
                        <input type="checkbox" value={"Посудомоечная машина"} id="checkbox30" onChange={() => ClickCheckbox(30)} className="checkBoxOpcii"/> 
                        <label htmlFor="checkbox30" className="textCheckboxOpcii">Посудомоечная машина </label>
                      </div>
                  </div>
                </div>
              ...
              <div className="second__block__delenie">
                ...
                <div className="vtoroe__delenie">
                  ...
                  <div className="vtoroe__delenie__info">
                    <Link to="/katalog/city=" className="vtoroe__delenie__links" onClick={()=>{dispatch(setCity("Минск")); dispatch(setKvartiri(""))}}>Квартиры в Минске</Link>
                    <p className="vtoroe__delenie__sec__text">{arendaLength}</p>
                  </div>
                  <div className="vtoroe__delenie__info">
                    <Link to="/katalog/city=" className="vtoroe__delenie__links" onClick={()=>{dispatch(setCity("Гомель")); dispatch(setKvartiri(""))}}>Квартиры в Гомеле</Link>
                    <p className="vtoroe__delenie__sec__text">{arendaLength2}</p>
                  </div>
                  <div className="vtoroe__delenie__info">
                    <Link to="/katalog/city=" className="vtoroe__delenie__links" onClick={()=>{dispatch(setCity("Гродно")); dispatch(setKvartiri(""))}}>Квартиры в Гродно</Link>
                    <p className="vtoroe__delenie__sec__text">{arendaLength3}</p>
                  </div>
                  <div className="vtoroe__delenie__info">
                    <Link to="/katalog/city=" className="vtoroe__delenie__links" onClick={()=>{dispatch(setCity("Могилев")); dispatch(setKvartiri(""))}}>Квартиры в Могилеве</Link>
                    <p className="vtoroe__delenie__sec__text">{arendaLength4}</p>
                  </div>
                  <div className="vtoroe__delenie__info">
                    <Link to="/katalog/city=" className="vtoroe__delenie__links"onClick={()=>{dispatch(setCity("Брест")); dispatch(setKvartiri(""))}}>Квартиры в Бресте</Link>
                    <p className="vtoroe__delenie__sec__text">{arendaLength5}</p>
                  </div>
                  <div className="vtoroe__delenie__info">
                    <Link to="/katalog/city=" className="vtoroe__delenie__links"onClick={()=>{dispatch(setCity("Витебск")); dispatch(setKvartiri(""))}}>Квартиры в Витебске</Link>
                    <p className="vtoroe__delenie__sec__text">{arendaLength6}</p>
                  </div>

                  <h2 className="vtoroe__delenie__inner">Коттеджи и усадьбы</h2>
                  <div className="vtoroe__delenie__info">
                    <Link to="123" className="vtoroe__delenie__links">Аггроусадьбы</Link>
                    <p className="vtoroe__delenie__sec__text">110</p>
                  </div>
                  <div className="vtoroe__delenie__info">
                    <Link to="123" className="vtoroe__delenie__links">Коттеджи</Link>
                    <p className="vtoroe__delenie__sec__text">110</p>
                  </div>
                  <div className="vtoroe__delenie__info">
                    <Link to="123" className="vtoroe__delenie__links">Загородный комплекс</Link>
                    <p className="vtoroe__delenie__sec__text">110</p>
                  </div>
                  <div className="vtoroe__delenie__info">
                    <Link to="123" className="vtoroe__delenie__links">База отдыха</Link>
                    <p className="vtoroe__delenie__sec__text">110</p>
                  </div>
                  <div className="vtoroe__delenie__info">
                    <Link to="123" className="vtoroe__delenie__links">
                      <div className="" style={{border:"1px solid #F5BC03",borderRight:"0",borderLeft:"0",borderTop:"0"}} onMouseEnter={click2} onMouseLeave={click2}>
                        <div className="dropdown4">
                          <span style={{marginRight:"5.5px",paddingBottom:"3px"}}>Еще</span>
                            ...
                            <ul className="Spisok__dropdown__custom">     
                  ...
                  <div className="vtoroe__delenie__info">
                    <Link to="123" className="vtoroe__delenie__links">Коттеджи и усадьбы на о. Брасласких </Link>
                    <p className="vtoroe__delenie__sec__text"></p>
                  </div>
                  <div className="vtoroe__delenie__info">
                    <Link to="123" className="vtoroe__delenie__links vtoroe__delenie__links2 ">Коттеджи и усадьбы (жилье) на Нарочи</Link>
                    <p className="vtoroe__delenie__sec__text"></p>
                  </div>
                  <div className="vtoroe__delenie__info">
                    <Link to="123" className="vtoroe__delenie__links">Коттеджи и усадьбы (жилье) у воды,
              ...
              <div className="Card">
                  <div className="ColRowHome conteiner" style={{position:"absolute",
                    ...
                    <div className="predlojenia">
                      ...
                      <Link to={`/katalog/city=`}>
                        <button className="btndob btnpredl" onClick={()=>{window.scroll(0,0); dispatch(setCity(filter.cityRayon.city))}}>Посмотреть все</button>
                      </Link>
                      </div>
                    </div>
            ...
            <div className="delenie__third__block">
              ...
              <div className="vtoroe__delenie__third__block">
                <div className="select_metro">
                      <div className="dropdown">
                        <button className="Spisok spisokmetro" onClick={click23}>{filter.cityRayon.city}</button>
                        <ul className="Spisok__dropdown">
                          <li className="dropdown__item" key={"Минск"}>Минск</li>
                          <li className="dropdown__item" key={"Гомель"}>Гомель</li>
                          <li className="dropdown__item" key={"Гродно"}>Гродно</li>
                      ...
                      <div className="dropdown">
                        <button className="Spisok spisokmetro" onClick={click23}>Выберите</button>
                        <ul className="Spisok__dropdown">
                        <>
                        {
                            Rayon.map((item)=>{
              ...
              <div className="delenie__flex">
                <div className="item__info__forth">
                  <div className="infoicon">
                    ...
                    <button className="Voyti razmestitbtn" onClick={()=>login?setModalActive2(true):notify2()}>+ Разместить объявление</button>
                </div>
      ...
      <Modal active={modalActive2} setActive={setModalActive2}>
          {
            login?
            <>
             <h1 style={{textAlign:"center", marginBottom:"15px"}} className="textBrith">Разместите Ваше объявление</h1>
             <section className="FirstObyablenie">
              <div className="infoObyavlenie">
                    ...
                    <Formik
                      initialValues={{
                        ...
                        <Form>
                          ...
                          <div className={(filter.categoryInfoId === 1 && (values.city!=="" && values.kolvo!=="" && values.komnati!=="" && values.linkViber!=="" && values.linkWats!=="" && values.linkMail!=="" && values.mail!=="" && values.metro!=="" && values.name!=="" && values.number!=="" && values.ploshad!=="" && values.rayon!=="" && values.sent!=="" && (!errors.sent) && (!errors.ploshad) && (!errors.mail) && (!errors.linkMail) && (imgUrl && imgUrl2)))? "tabs tabs__active":(filter.categoryInfoId === 1 && (values.city=="" || values.kolvo=="" || values.komnati=="" || values.linkViber=="" || values.linkWats=="" || values.linkMail=="" || values.mail=="" || values.metro=="" || values.name=="" || values.number=="" || values.ploshad=="" || values.rayon=="" || values.sent=="" || (errors.sent) || (errors.ploshad) || (errors.mail) || (errors.linkMail) || (!imgUrl || !imgUrl2)))?"tabs tabs__active tabs__error__active":(filter.categoryInfoId !== 1 && (values.city=="" || values.kolvo=="" || values.komnati=="" || values.linkViber=="" || values.linkWats=="" || values.linkMail=="" || values.mail=="" || values.metro=="" || values.name=="" || values.number=="" || values.ploshad=="" || values.rayon=="" || values.sent=="" || (errors.sent) || (errors.ploshad) || (errors.mail) || (errors.linkMail) || (!imgUrl || !imgUrl2))?"tabs tabs__error":"tabs" )} onClick={()=>dispatch(setCategoryInfoId(1))} >
                            ...
                            <div className={(filter.categoryInfoId === 3 && values.opisanie!=="") ? "tabs tabs__active":(filter.categoryInfoId===3 && values.opisanie=="")?"tabs tabs__active tabs__error__active":(filter.categoryInfoId!==3 && values.opisanie=="")?"tabs tabs__error":"tabs"} onClick={()=>dispatch(setCategoryInfoId(3))}>
                              <p className="select__item" >Описание Вашего объявления*</p>
                            </div>
                          </div>  
                          
                          <div className={filter.categoryInfoId===1?"delenieObyavlenie":"delenieObyavlenieNone"} style={{marginBottom:"15px"}}>
                            <div className="partOneObyavlenie" style={{marginRight:"15px"}}>
                              <h2>Фото Вашего объекта</h2>
                              <img className={imgUrl?"IconImgObyavlenie":"IconImgObyavlenie imgerror"} src={imgUrl?imgUrl:"https://cdn.dribbble.com/users/2657768/screenshots/6413526/404_43.gif"} alt="" />
                                <button onClick={btnChoose} className="Voyti choose" type="button">Выберите файл</button>
                              <Field id="file-input4" type="file" onChange={handleOnChange} value={values.url} style={{display:"none"}} />
                            </div>
                            <div className="partTwoObyavlenie" style={{display:"flex"}}>
                              ...
                              <div className="LOGIN">
                                <Field className={((touched.komnati && errors.komnati) ||(values.komnati===""))? "login__error cityArenda":"login cityArenda"} onChange={handleChange} onBlur={handleBlur} name = "komnati" type="text" value={values.komnati} placeholder="Комнаты"/>
                                <svg className={((touched.komnati && errors.komnati)||(values.komnati===""))? "icon__error":"iconHidden"}  width="21" height="20" viewBox="0 0 21 20" fill="rgb(235, 87, 87)" xmlns="http://www.w3.org/2000/svg">
                                  <path d="M10.5 0C5 0 0.5 4.5 0.5 10C0.5 15.5 5 20 10.5 20C16 20 20.5 15.5 20.5 10C20.5 4.5 16 0 10.5 0ZM10.5 2C11.6 2 12.4 2.9 12.3 4L11.5 12H9.5L8.7 4C8.6 2.9 9.4 2 10.5 2ZM10.5 18C9.4 18 8.5 17.1 8.5 16C8.5 14.9 9.4 14 10.5 14C11.6 14 12.5 14.9 12.5 16C12.5 17.1 11.6 18 10.5 18Z"/>
                                </svg>
                              ...
                              <div className="LOGIN">
                                <Field className={((touched.kolvo && errors.kolvo)||(values.kolvo===""))? "login__error cityArenda":"login cityArenda"} onChange={handleChange} onBlur={handleBlur} name = "kolvo" type="text" value={values.kolvo} placeholder="Кол-во людей"/>
                                <svg className={((touched.kolvo && errors.kolvo)||(values.kolvo===""))? "icon__error":"iconHidden"}  width="21" height="20" viewBox="0 0 21 20" fill="rgb(235, 87, 87)" xmlns="http://www.w3.org/2000/svg">
                                  <path d="M10.5 0C5 0 0.5 4.5 0.5 10C0.5 15.5 5 20 10.5 20C16 20 20.5 15.5 20.5 10C20.5 4.5 16 0 10.5 0ZM10.5 2C11.6 2 12.4 2.9 12.3 4L11.5 12H9.5L8.7 4C8.6 2.9 9.4 2 10.5 2ZM10.5 18C9.4 18 8.5 17.1 8.5 16C8.5 14.9 9.4 14 10.5 14C11.6 14 12.5 14.9 12.5 16C12.5 17.1 11.6 18 10.5 18Z" fill="#EB5757"/>
                                </svg>
                              ...
                              <div className="LOGIN">
                                <Field className={((touched.ploshad && errors.ploshad)||(values.ploshad==""))? "login__error cityArenda":"login cityArenda"} onChange={handleChange} onBlur={handleBlur} name = "ploshad" type="text" value={values.ploshad} placeholder="Площадь объекта"/>
                                <svg className={((touched.ploshad && errors.ploshad)||(values.ploshad==""))? "icon__error":"iconHidden"}  width="21" height="20" viewBox="0 0 21 20" fill="rgb(235, 87, 87)" xmlns="http://www.w3.org/2000/svg">
                                  <path d="M10.5 0C5 0 0.5 4.5 0.5 10C0.5 15.5 5 20 10.5 20C16 20 20.5 15.5 20.5 10C20.5 4.5 16 0 10.5 0ZM10.5 2C11.6 2 12.4 2.9 12.3 4L11.5 12H9.5L8.7 4C8.6 2.9 9.4 2 10.5 2ZM10.5 18C9.4 18 8.5 17.1 8.5 16C8.5 14.9 9.4 14 10.5 14C11.6 14 12.5 14.9 12.5 16C12.5 17.1 11.6 18 10.5 18Z"/>
                                </svg>
                          ...
                          <div className="partThreeObyavlenie" style={{marginLeft:"15px"}}>
                            <h2>Ваше фото</h2>
                            <img className={imgUrl2?"IconImgObyavlenie":"IconImgObyavlenie imgerror"} src={imgUrl2?imgUrl2:"https://cdn.dribbble.com/users/2657768/screenshots/6413526/404_43.gif"} alt="" />
                            <button onClick={btnChoose2} type="button" className="Voyti choose">Выберите файл</button>
                            <Field id="file-input5" type="file" value={values.imageVladelec} onChange={handleOnChange2} style={{display:"none"}} />
                          </div>
                        </div>
                        <div className={filter.categoryInfoId===2?"DopInfo":"DopInfoNone"}>
                          <div className="OsnovaOpcii" style={{marginBottom:"20px"}}>
                            {/* Первое деление */}
                            <div className="delenieOpcii">
                              <p className="titleOpcii">Спальные места</p>
                              <div className="dropdown">
                                <button className="Spisok" id="FilterMestaDop" type="button" onClick={()=>click()}>Выберите</button>
                                <ul className="Spisok__dropdown">
                                  <li key={"1 место"} className="dropdown__item">1 место</li>
                                  <li key={"2 место"} className="dropdown__item">2 места</li>
                                  <li key={"3 место"} className="dropdown__item">3 места</li>
                              ...
                              <div className="checkbox" style={{display:"flex",marginBottom:"10px",marginTop:"30px"}}>
                                <input type="checkbox" value={"Газовая плита"} id="checkbox_module1" onChange={() => ClickCheckbox(1)} className="checkBoxOpcii2"/> 
                                <label htmlFor="checkbox_module1" className="textCheckboxOpcii">Газовая плита</label>
                              </div>
                              <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                <input type="checkbox" value={"Духовка"} id="checkbox_module2" onChange={() => ClickCheckbox(2)} className=" checkBoxOpcii2"/> 
                                <label htmlFor="checkbox_module2" className="textCheckboxOpcii">Духовка</label>
                              </div>
                              <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                <input type="checkbox" value={"Кофеварка"} id="checkbox_module3" onChange={() => ClickCheckbox(3)} className=" checkBoxOpcii2"/> 
                                <label htmlFor="checkbox_module3" className="textCheckboxOpcii">Кофеварка</label>
                              </div>
                              <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                <input type="checkbox" value={"Микроволновая печь"} id="checkbox_module4" onChange={() => ClickCheckbox(4)} className=" checkBoxOpcii2"/> 
                                <label htmlFor="checkbox_module4" className="textCheckboxOpcii">Микроволновая печь </label>
                              </div>
                              <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                <input type="checkbox" value={"Посуда"} id="checkbox_module5" onChange={() => ClickCheckbox(5)} className=" checkBoxOpcii2"/> 
                                <label htmlFor="checkbox_module5" className="textCheckboxOpcii">Посуда </label>
                              </div>
                              <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                <input type="checkbox" value={"Посудомоечная машина"} id="checkbox_module6" onChange={() => ClickCheckbox(6)} className=" checkBoxOpcii2"/> 
                                <label htmlFor="checkbox_module6" className="textCheckboxOpcii">Посудомоечная машина </label>
                              </div>
                            </div>
                          {/* Второе деление */}
                            <div className="delenieOpcii">
                              <p className="titleOpcii"style={{visibility:"hidden"}}>Район</p>
                              <div className="dropdown">
                                <button style={{visibility:"hidden"}} className="Spisok" type="button">Выберите</button>
                                <input type="text" name="select__category" value="" className="drodown__item__hiden" />
                              </div>
                              <div className="checkbox" style={{display:"flex",marginBottom:"10px",marginTop:"30px"}}>
                                <input type="checkbox" value={"Газовая плита"} id="checkbox_module7" onChange={() => ClickCheckbox(7)} className=" checkBoxOpcii2"/> 
                                <label htmlFor="checkbox_module7" className="textCheckboxOpcii">Газовая плита</label>
                              </div>
                              <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                <input type="checkbox" value={"Духовка"} id="checkbox_module8" onChange={() => ClickCheckbox(8)} className=" checkBoxOpcii2"/> 
                                <label htmlFor="checkbox_module8" className="textCheckboxOpcii">Духовка</label>
                              </div>
                              <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                <input type="checkbox" value={"Кофеварка"} id="checkbox_module9" onChange={() => ClickCheckbox(9)} className=" checkBoxOpcii2"/> 
                                <label htmlFor="checkbox_module9" className="textCheckboxOpcii">Кофеварка</label>
                              </div>
                              <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                <input type="checkbox" value={"Микроволновая печь"} id="checkbox_module10" onChange={() => ClickCheckbox(10)} className=" checkBoxOpcii2"/> 
                                <label htmlFor="checkbox_module10" className="textCheckboxOpcii">Микроволновая печь </label>
                              </div>
                              <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                <input type="checkbox" value={"Посуда"} id="checkbox_module11" onChange={() => ClickCheckbox(11)} className=" checkBoxOpcii2"/> 
                                <label htmlFor="checkbox_module11" className="textCheckboxOpcii">Посуда </label>
                              </div>
                              <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                <input type="checkbox" value={"Посудомоечная машина"} id="checkbox_module12" onChange={() => ClickCheckbox(12)} className=" checkBoxOpcii2"/> 
                                <label htmlFor="checkbox_module12" className="textCheckboxOpcii">Посудомоечная машина </label>
                              </div>
                            </div>
                            {/* Третие делене */}
                            <div className="delenieOpcii">
                              ...
                              <div className="checkbox" style={{display:"flex",marginBottom:"10px",marginTop:"30px"}}>
                                <input type="checkbox" value={"Газовая плита"} id="checkbox_module13" onChange={() => ClickCheckbox(13)} className=" checkBoxOpcii2"/> 
                                <label htmlFor="checkbox_module13" className="textCheckboxOpcii">Газовая плита</label>
                              </div>
                              <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                <input type="checkbox" value={"Духовка"} id="checkbox_module14" onChange={() => ClickCheckbox(14)} className=" checkBoxOpcii2"/> 
                                <label htmlFor="checkbox_module14" className="textCheckboxOpcii">Духовка</label>
                              </div>
                              <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                <input type="checkbox" value={"Кофеварка"} id="checkbox_module15" onChange={() => ClickCheckbox(15)} className=" checkBoxOpcii2"/> 
                                <label htmlFor="checkbox_module15" className="textCheckboxOpcii">Кофеварка</label>
                              </div>
                              <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                <input type="checkbox" value={"Микроволновая печь"} id="checkbox_module16" onChange={() => ClickCheckbox(16)} className=" checkBoxOpcii2"/> 
                                <label htmlFor="checkbox_module16" className="textCheckboxOpcii">Микроволновая печь </label>
                              </div>
                              <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                <input type="checkbox" value={"Посуда"} id="checkbox_module17" onChange={() => ClickCheckbox(17)} className=" checkBoxOpcii2"/> 
                                <label htmlFor="checkbox_module17" className="textCheckboxOpcii">Посуда </label>
                              </div>
                              <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                <input type="checkbox" value={"Посудомоечная машина"} id="checkbox_module18" onChange={() => ClickCheckbox(18)} className=" checkBoxOpcii2"/> 
                                <label htmlFor="checkbox_module18" className="textCheckboxOpcii">Посудомоечная машина </label>
                              </div>
                            </div>
                          {/* Четвертое деление */}
                            <div className="delenieOpcii">
                            ...
                            <p className="titleOpcii">Метро</p>
                              ...
                              <div className="checkbox" style={{display:"flex",marginBottom:"10px",marginTop:"30px"}}>
                                  <input type="checkbox" value={"Газовая плита"} id="checkbox_module19" onChange={() => ClickCheckbox(19)} className=" checkBoxOpcii2"/> 
                                  <label htmlFor="checkbox_module19" className="textCheckboxOpcii">Газовая плита</label>
                                </div>
                                <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                  <input type="checkbox" value={"Духовка"} id="checkbox_module20" onChange={() => ClickCheckbox(20)} className=" checkBoxOpcii2"/> 
                                  <label htmlFor="checkbox_module20" className="textCheckboxOpcii">Духовка</label>
                                </div>
                                <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                  <input type="checkbox" value={"Кофеварка"} id="checkbox_module21" onChange={() => ClickCheckbox(21)} className=" checkBoxOpcii2"/> 
                                  <label htmlFor="checkbox_module21" className="textCheckboxOpcii">Кофеварка</label>
                                </div>
                                <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                  <input type="checkbox" value={"Микроволновая печь"} id="checkbox_module22" onChange={() => ClickCheckbox(22)} className=" checkBoxOpcii2"/> 
                                  <label htmlFor="checkbox_module22" className="textCheckboxOpcii">Микроволновая печь </label>
                                </div>
                                <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                  <input type="checkbox" value={"Посуда"} id="checkbox_module23" onChange={() => ClickCheckbox(23)} className=" checkBoxOpcii2"/> 
                                  <label htmlFor="checkbox_module23" className="textCheckboxOpcii">Посуда </label>
                                </div>
                                <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                  <input type="checkbox" value={"Посудомоечная машина"} id="checkbox_module24" onChange={() => ClickCheckbox(24)} className=" checkBoxOpcii2"/> 
                                  <label htmlFor="checkbox_module24" className="textCheckboxOpcii">Посудомоечная машина </label>
                                </div>
                            </div>
                          {/* Пятое деление */}
                            <div className="delenieOpcii">
                              <div className="dropdown" style={{visibility: "hidden"}}>
                                <p className="titleOpcii" >Метро</p>
                                <button className="Spisok" id = "FilterMetro" type="button" onClick={click}>Выберите</button>
                                <ul className="Spisok__dropdown">
                                  <li key={"Грушевка"} className="dropdown__item">Грушевка</li>
                                </ul>
                                <input type="text" name="select__category"  className="drodown__item__hiden" />
                              ...
                              <div className="checkbox" style={{display:"flex",marginBottom:"10px",marginTop:"30px"}}>
                                  <input type="checkbox" value={"Газовая плита"} id="checkbox_module25" onChange={() => ClickCheckbox(25)} className=" checkBoxOpcii2"/> 
                                  <label htmlFor="checkbox_module25" className="textCheckboxOpcii">Газовая плита</label>
                                </div>
                                <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                  <input type="checkbox" value={"Духовка"} id="checkbox_module26" onChange={() => ClickCheckbox(26)} className=" checkBoxOpcii2"/> 
                                  <label htmlFor="checkbox_module26" className="textCheckboxOpcii">Духовка</label>
                                </div>
                                <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                  <input type="checkbox" value={"Кофеварка"} id="checkbox_module27" onChange={() => ClickCheckbox(27)} className=" checkBoxOpcii2"/> 
                                  <label htmlFor="checkbox_module27" className="textCheckboxOpcii">Кофеварка</label>
                                </div>
                                <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                  <input type="checkbox" value={"Микроволновая печь"} id="checkbox_module28" onChange={() => ClickCheckbox(28)} className=" checkBoxOpcii2"/> 
                                  <label htmlFor="checkbox_module28" className="textCheckboxOpcii">Микроволновая печь </label>
                                </div>
                                <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                  <input type="checkbox" value={"Посуда"} id="checkbox_module29" onChange={() => ClickCheckbox(29)} className=" checkBoxOpcii2"/> 
                                  <label htmlFor="checkbox_module29" className="textCheckboxOpcii">Посуда </label>
                                </div>
                                <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                  <input type="checkbox" value={"Посудомоечная машина"} id="checkbox_module30" onChange={() => ClickCheckbox(30)} className=" checkBoxOpcii2"/> 
                                  <label htmlFor="checkbox_module30" className="textCheckboxOpcii">Посудомоечная машина </label>
                                </div>
                            </div>
                          </div>
                        </div>
                        <div className={filter.categoryInfoId===3?"DopInfo":"DopInfoNone"}>
                          <div className="Message">
                            <p className="kontakti__text">Описание объявления</p>
                              <textarea name="opisanie" className={(values.opisanie=="")?"TextAreaMes__error":"TextAreaMes"} onChange={handleChange} onBlur={handleBlur} value={values.opisanie} placeholder="Описание" cols={30} rows={10}></textarea>
                          </div>
                        </div>
                        <div className="" style={{display:"flex",justifyContent:"center"}}>
                            <div style={{width:"fit-content",padding:"9px 16px"}} className={("OshibkaVvoda" && ((touched.sent && errors.sent) || (touched.ploshad && errors.ploshad) || (touched.mail && errors.mail) || (touched.linkMail && errors.linkMail) || (values.city=="" || values.kolvo=="" || values.komnati=="" || values.linkViber=="" || values.linkWats=="" || values.linkMail=="" || values.mail=="" || values.metro=="" || values.name=="" || values.number=="" || values.ploshad=="" || values.rayon=="" || values.sent=="" || !imgUrl || !imgUrl2)))? "OshibkaVvoda fix":"OshibkaVvoda"}>
                              <p className="OshibkaVvodaText">Не все поля заполнены</p>
                              <div className="">
                        ...
                        <div className="" style={{display:"flex",justifyContent:"center"}}>
                          <button className="Voyti choose" type="submit" disabled={(values.city=="" || (errors.mail) || (errors.sent) || (errors.ploshad) || (errors.linkMail) || values.kolvo=="" || values.komnati=="" || values.linkViber=="" || values.linkWats=="" || values.linkMail=="" || values.mail=="" || values.metro=="" || values.name=="" || values.number=="" || values.ploshad=="" || values.rayon=="" || values.sent=="" || (imgUrl && imgUrl2))?!isValid:isValid}  onClick={()=>{handleSubmit}}>Разместить объявление</button>
                        </div>
                        </Form>
                      )}
                    </Formik>
