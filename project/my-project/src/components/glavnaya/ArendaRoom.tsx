...
const ArendaRoom=()=>{
  ...
  const settings = {
    ...
    const push = (item,city2) =>{
      navigate(`/katalog/city=${city2}/${item}`);
      window.scroll(0,0)
    }
  ...
  useEffect(()=>{
    ...
    if (window.location.search) {
      ...
      const item = [params.cityFilter,params.rayonFilter]
      ...
      setLoading(true);
      axios.get(`/ArendaCard?city2=${filterCityRayon.city}&rayon=${filterCityRayon.rayon}`).then(({data})=>{
        const item = [filterCityRayon.city,filterCityRayon.rayon,data.length];
        dispatch(setCityRayonHomePage(item));
        setArenda(data);
        setLoading(false);
     })
      ...
      <Card key={item.id} className="card__style" style={{width:"406px",height:"535px"}}>
        <div className={"SpisokInformKontakti"}>
          <div className="CardOsnova">
            ...
            <div className="">
              <AwesomeSlider> 
                <Card.Img  data-src={item.url} variant="top" className="imgCard" src={item.url}/>
              </AwesomeSlider>   
            </div> 
