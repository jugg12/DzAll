...
import { clearFilter,setCity,setFilterAll,setSort } from "../../store/slices/FilterSlice";
          ...
          <div className="part__second__footer">
            <div className="fisrt__part__sec__footer">
              ...
              <div className="info2">
                ...
                <div className="info2__delenie">
                  <div className="delenie1">
                  {
                    Cities.slice(0,delenie).map((item)=>{
                      return(
                  ...
                  <div className="delenie1 delenie2">
                    {
                      Cities.slice(delenie,Cities.length).map((item)=>{
                        return(
                          <p key={item.city} className="info2__text" onClick={()=>push2(`${item.city}`,`${cityIn(item.city)}`)}>Квартиры в {cityIn(item.city)}</p>
                        )
                      })  
              ...
              <div className="info3">
                <Link to = "/News" className="info3__text" onClick={()=>window.scroll(0,0)}>Новости</Link>
                <Link to = "" className="info3__text" onClick={()=>window.scroll(0,0)}>Размещение и тарифы</Link>
                <Link to = "/Map" className="info3__text" onClick={()=>window.scroll(0,0)}>Объявления на карте</Link>
                <Link to = "/kontakti" className="info3__text" onClick={()=>window.scroll(0,0)}>Контакты</Link>
              </div>
            </div>
