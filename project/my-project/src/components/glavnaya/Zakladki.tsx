...
import { setValue } from "../../store/actions/zakladkiAction";

export default function Zakladki() {
  const history=useNavigate();
  const login = localStorage.getItem("login");
  const [zakladki, setZakladki] = useState<any>([]);
  const [zakladkiChange, setZakladkiChange] = useState<any>([]);
  const gorod = localStorage.setItem("gorod", "Минск");
  ...
  const kvartiri = localStorage.setItem(
    "kvartiri",
    "Квартиры на сутки"
  );
  ...
  console.log(zakladkiChange);

  const handlepageclick = (e) =>{
    const newOffset = (e.selected*KatalogPerPage)%zakladkiChange.length;
    ...
    if(login){
      axios.get(`/users?login=${login}`).then((data) => {
        for (let i = 0; i < data.data[0].zakladki.length; i++) {
          if(data.data[0].zakladki[i].id==value && data2==true){
            notify();
            data.data[0].zakladki.splice(i,1);
            axios.patch(`/users/${id}`,{"zakladki":data.data[0].zakladki});
          }
        }
        setZakladkiChange(data.data[0].zakladki)
        setData2(false);
      });
    }
  },[data2,login]);
  ...
  useEffect(()=>{
    ...
    setCountPage(Math.ceil(zakladkiChange.length/KatalogPerPage));
  },[itemOffset,KatalogPerPage,zakladkiChange])
          ...
          <div className="conteiner">
            <div className="hlebnie">
              <nav className="breadcrumbs" style={{ display: "flex" }}>
                ...
                <Link to="/Zakladki" style={{ textDecoration: "none" }}>
          ...
          <div className="conteiner">
            <div className="ZakladkiOsnova">
              {login?(
                            ...
                            <Card.Body className="bodyCard" >
                              <Card.Title className="card__title">
                                <div className="perviytitle">
                                  ...
                                  <div className="kolvoludey Ploshad">{item.ploshad}<sup style={{lineHeight:"5px"}}><small>2</small></sup></div>
                                </div>
               ...
               : (
                <div className="Soobshenie">
                  <h1>Необходимо <Link to="/vhod">авторизоваться</Link></h1>
