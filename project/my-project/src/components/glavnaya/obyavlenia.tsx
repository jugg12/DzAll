...
import CardSkeleton from "../glavnaya/CardSkeleton";
...
export default function obyavlenia() {
  const history=useNavigate();
  const login = localStorage.getItem("login");
  const [obyavlenia, setObyavlenia] = useState<any>([]);
  const [obyavleniaChange, setObyavleniaChange] = useState<any>([]);
  const id = localStorage.getItem("id");
  const [currentPage,setCurrentPage] = useState(null);
  const [KatalogPerPage,setKatalogPerPage]= useState(6)
  ...
  const [komnatiInfo,setKomnatiInfo] = useState<string>();
  const [kolvoInfo,setKolvoInfo] = useState<string>();
  const [ploshadInfo,setPloshadInfo] = useState<any>();
  const [metroInfo,setMetroInfo] = useState<string>();
  const [rayonInfo,setRayonInfo] = useState<string>();
  const [opisanieInfo,setOpisanieInfo] = useState<string>();
  const [urlInfo,setUrlInfo] = useState<any>("");
  const [imageVladelecInfo,setImageVladelecInfo] = useState<string>();
  const [nameInfo,setNameInfo] = useState<string>();
  const [numberInfo,setNumberInfo] = useState<string>();
  const [mailInfo,setMailInfo] = useState<string>();
  const [linkViberInfo,setLinkViberInfo] = useState<string>();
  const [linkMailInfo,setLinkMailInfo] = useState<string>();
  const [linkWatsInfo,setLinkWatsInfo] = useState<string>();
  const [dopNames,setDopNames] = useState<string>();
  ...
  // push-увдомление
  const notify = () => {
    toast.success("Ваше объявление изменено",{
      position:"top-center",
      closeOnClick:true,
      pauseOnHover:false,
      autoClose:2000,
  ...
  const clickHeart = (value,item)=>{ //Клик на изменение
  ...
  fileReader.onloadend = () => {
    ...
    setUrl1(fileReader.result)
  }

  fileReader2.onloadend = () => {
    setImgUrl2(fileReader2.result);
    setUrl2(fileReader2.result)
  }
  ...
  const handleOnChange9 = (e) => {
    const result = e.target.value;
    setOpisanieInfo(result);
  }

  const handleOnChange11 = (e) => {
    const result = e.target.value;
    setImageVladelecInfo(result);
  }
  ...
  const handlepageclick = (e) =>{
    const newOffset = (e.selected*KatalogPerPage)%obyavleniaChange.length;
    setItemOffset(newOffset)
  }

  const push = (item) =>{
    history(`/obyavlenia/test/${item-1}`)
  }

  useEffect(() => {
    let value = localStorage.getItem("value2");
    if (login){
      axios.get(`/users?login=${login}`).then((data) => {
        for (let i = 0; i < data.data[0].obyavlenia.length; i++) {
          if(data.data[0].obyavlenia[i].id==value && data2==true){
            setItemInfo(data.data[0].obyavlenia[i]);
          }
        }
        setObyavleniaChange(data.data[0].obyavlenia);
        setData2(false);
      })
    }
  },[data2,obyavleniaChange.length]);
  ...
  useEffect(() => {
    ...
    axios.get(`/users?login=${login}`).then((data) => {
      for (let i = 0; i < data.data[0].obyavlenia.length; i++) {
        if(data.data[0].obyavlenia[i].id==value && data2==false && itemInfo!==undefined){
            setIdInfo(itemInfo.id)
            setCityInfo(itemInfo.city);
            setSentInfo(itemInfo.sent);
            setKomnatiInfo(itemInfo.komnati);
            setKolvoInfo(itemInfo.kolvo);
            setPloshadInfo(itemInfo.ploshad);
            setMetroInfo(itemInfo.metro);
            setRayonInfo(itemInfo.rayon);
            setOpisanieInfo(itemInfo.opisanie);
            setUrlInfo(itemInfo.url);
            setImageVladelecInfo(itemInfo.imageVladelec);
            ...
            setDopNames(itemInfo.opcii[0].name);
            setMesta(itemInfo.opcii[0].mesta);
            setImgUrl("");
            setImgUrl2("");
            CloneInfo = [];
        }
  ...
  useEffect(()=>{
    ...
    setCurrentPage(obyavleniaChange.slice(itemOffset,endoffset));
    setCountPage(Math.ceil(obyavleniaChange.length/KatalogPerPage));
  },[itemOffset,KatalogPerPage,obyavleniaChange])
  ...
  const addArenda3 = () => {
    ...
    if(vibor==true){
      const item = {
        ...
        "metro":metroInfo,
        "rayon":rayonInfo,
        "opisanie":opisanieInfo,
        "url":urlInfo,
        "imageVladelec":imageVladelecInfo,
        "name":nameInfo,
        "number":numberInfo,
        "mail":mailInfo,
      ...
      obyavleniaChange.map((itemClone)=>{
  ...
  return (
    <>
      <Header />
      <section className="firstKatalog">
          ...
          <div className="conteiner">
            <div className="hlebnie">
              <nav className="breadcrumbs" style={{ display: "flex" }}>
                ...
                <Link to="/obyavlenia" style={{ textDecoration: "none" }}>
      ...
      <section className="SecondZakladki" style={{marginTop:"50px"}}>
        <div className="CardsZakladki">
          <div className="conteiner">
            <div className="ZakladkiOsnova">
              {obyavleniaChange.length ? (
                      ...
                      <Card className="card__style" style={{width:"406px",height:"535px"}}>
                        <div className="SpisokInformKontakti">  
                          <div className="CardOsnova">
                            <div className="Gold proverka">
                              <div className="colorProverka">
                                {
                                item.proverka==="На рассмотрении"?
                                <p className="Rassmotrenie">{item.proverka}</p>:
                                <p className={item.proverka=="Добавлено"?"Dobavleno":"Otkaz"}>{item.proverka}</p>
                                }
                              </div>
                            </div>
                            ...
                            <Card.Body className="bodyCard" >
                              <Card.Title className="card__title">
                                <div className="perviytitle">
                                  <div className="stoimostobshaya">
                                    <div className="stoimost">{item.sent}.00 BYN</div>
                                    <p className="info__under">за сутки</p>
                                  </div>
                                  <div className="kolvoludey">
                                    <div className="iconludi">
                                    ...
                                    <div className="chisloludey">{item.kolvo}</div>
                                  </div>
                                  <div className="kolvoludey komnati"><span className="kolvoKomtan">{item.komnati}</span></div>
                                  <div className="kolvoludey Ploshad">{item.ploshad} м<sup style={{lineHeight:"5px"}}><small>2</small></sup></div>
                                </div>
                                <div className="vtoroytitle">
                              ...
                              <Card.Text className="card__text">
                                ...
                                <div className="btnkontaktiOsnova" style={{marginTop:"80px"}}>
                                <div className="change" id={`change-${item.id}`} onClick={()=>clickHeart(item.id,item)}>
                                <svg className="change__pancil" viewBox="0 0 80 80" width="20" height="20" xmlSpace="preserve" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink">
                                  ...
                                  <div className="dropdownKontakti">
                                    <button className="kontaktibtn kontaktibtn2" id={item.id} onClick={()=>btnclick(item.id)}>
                                ...
                                <p className="Vladelec">Владелец</p>
                                <div className="NameVladels">
                                <h1 className="NameVladels">{item.name}</h1>
                                </div>
                                <div className="NumberVladels">
                                  <h1 className="NumberVladels">{item.number}</h1>
                                </div>
                                <div className="pochtaVladels">
                                  <a className="pochtaVladels"href={item.mail}>{item.mail}</a>
                                </div>
                                <div className="links">
                                  <a className="iconSocKontakti" href={item.linkViber}>
                                  ...
                                  <a className="iconSocKontakti" href={item.linkWats}>
                                    <svg width="36" height="36" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg">
                                  ...
                                  <a className="iconSocKontakti" href={item.linkMail}>
                                  <svg width="36" height="36" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <div className="Podrobnee">
                                    <p className="textPodrobnee" onClick={(e)=>push(item.id)}>Подробнее</p>
                                </div>
                                </Button>
                              </div>
                            </Card.Body>
      ...
      {
          <>
             <h1 style={{textAlign:"center", marginBottom:"15px"}} className="textBrith">Редактирование Вашего объявления</h1>
                      ...
                      <Formik 
                        ...
                        <Form>
                          <div className="block__tabs" style={{justifyContent:"center",marginBottom:"20px"}}>
                            <div className={(ToggleState === 1 && (cityInfo!=="" && kolvoInfo!=="" && komnatiInfo!=="" && linkViberInfo!=="" && linkWatsInfo!=="" && linkMailInfo!=="" && mailInfo!=="" && metroInfo!=="" && nameInfo!=="" && numberInfo!=="" && ploshadInfo!=="" && rayonInfo!=="" && sentInfo!=="" ))? "tabs tabs__active":(ToggleState === 1 && (cityInfo=="" || kolvoInfo=="" || komnatiInfo=="" || linkViberInfo=="" || linkWatsInfo=="" || linkMailInfo=="" || mailInfo=="" || metroInfo=="" || nameInfo=="" || numberInfo=="" || ploshadInfo=="" || rayonInfo=="" || sentInfo==""))?"tabs tabs__active tabs__error__active":(ToggleState !== 1 && (cityInfo=="" || kolvoInfo=="" || komnatiInfo=="" || linkViberInfo=="" || linkWatsInfo=="" || linkMailInfo=="" || mailInfo=="" || metroInfo=="" || nameInfo=="" || numberInfo=="" || ploshadInfo=="" || rayonInfo=="" || sentInfo=="")?"tabs tabs__error":"tabs" )} onClick={()=>toggleTab(1)} >
                            ...
                            <div className={(ToggleState === 3 && opisanieInfo!=="") ? "tabs tabs__active":(ToggleState===3 && opisanieInfo=="")?"tabs tabs__active tabs__error__active":(ToggleState!==3 && opisanieInfo=="")?"tabs tabs__error":"tabs"} onClick={()=>toggleTab(3)}>
                              <p className="select__item" >Описание Вашего объявления*</p>
                            </div>
                          </div>  
                          <div className={ToggleState===1?"delenieObyavlenie":"delenieObyavlenieNone"} style={{marginBottom:"15px"}}>
                            <div className="partOneObyavlenie" style={{marginRight:"15px"}}>
                              <h2>Фото Вашего объекта</h2>
                              <img className="IconImgObyavlenie"src={imgUrl?imgUrl:urlInfo} alt="" />
                                <button onClick={btnChoose} className="Voyti choose" type="button">Выберите файл</button>
                                <Field id="file-input3" type="file" onChange={handleOnChange} value={values.url} style={{display:"none"}} />
                            </div>
                            <div className="partTwoObyavlenie" style={{display:"flex"}}>
                              ...
                              <div className="LOGIN">
                                <Field className={(komnatiInfo==="")? "login__error cityArenda":"login cityArenda"}  name = "komnati" type="text" value={komnatiInfo} onChange={handleOnChange5} placeholder="Комнаты"/>
                                <svg className={(komnatiInfo==="")? "icon__error":"iconHidden"}  width="21" height="20" viewBox="0 0 21 20" fill="rgb(235, 87, 87)" xmlns="http://www.w3.org/2000/svg">
                                  <path d="M10.5 0C5 0 0.5 4.5 0.5 10C0.5 15.5 5 20 10.5 20C16 20 20.5 15.5 20.5 10C20.5 4.5 16 0 10.5 0ZM10.5 2C11.6 2 12.4 2.9 12.3 4L11.5 12H9.5L8.7 4C8.6 2.9 9.4 2 10.5 2ZM10.5 18C9.4 18 8.5 17.1 8.5 16C8.5 14.9 9.4 14 10.5 14C11.6 14 12.5 14.9 12.5 16C12.5 17.1 11.6 18 10.5 18Z"/>
                                </svg>
                              ...
                              <div className="LOGIN">
                                <Field className={(kolvoInfo==="")? "login__error cityArenda":"login cityArenda"}  name = "kolvo" type="text" value={kolvoInfo} onChange={handleOnChange18} placeholder="Кол-во людей"/>
                                <svg className={(kolvoInfo==="")? "icon__error":"iconHidden"}  width="21" height="20" viewBox="0 0 21 20" fill="rgb(235, 87, 87)" xmlns="http://www.w3.org/2000/svg">
                                  <path d="M10.5 0C5 0 0.5 4.5 0.5 10C0.5 15.5 5 20 10.5 20C16 20 20.5 15.5 20.5 10C20.5 4.5 16 0 10.5 0ZM10.5 2C11.6 2 12.4 2.9 12.3 4L11.5 12H9.5L8.7 4C8.6 2.9 9.4 2 10.5 2ZM10.5 18C9.4 18 8.5 17.1 8.5 16C8.5 14.9 9.4 14 10.5 14C11.6 14 12.5 14.9 12.5 16C12.5 17.1 11.6 18 10.5 18Z" fill="#EB5757"/>
                                </svg>
                              ...
                              <div className="LOGIN">
                                <Field className={(ploshadInfo==="")? "login__error cityArenda":"login cityArenda"}  name = "ploshad" type="text" value={ploshadInfo} onChange={handleOnChange6} placeholder="Площадь объекта"/>
                                <svg className={(ploshadInfo==="")? "icon__error":"iconHidden"}  width="21" height="20" viewBox="0 0 21 20" fill="rgb(235, 87, 87)" xmlns="http://www.w3.org/2000/svg">
                                  <path d="M10.5 0C5 0 0.5 4.5 0.5 10C0.5 15.5 5 20 10.5 20C16 20 20.5 15.5 20.5 10C20.5 4.5 16 0 10.5 0ZM10.5 2C11.6 2 12.4 2.9 12.3 4L11.5 12H9.5L8.7 4C8.6 2.9 9.4 2 10.5 2ZM10.5 18C9.4 18 8.5 17.1 8.5 16C8.5 14.9 9.4 14 10.5 14C11.6 14 12.5 14.9 12.5 16C12.5 17.1 11.6 18 10.5 18Z"/>
                                </svg>
                          ...
                          <div className="partThreeObyavlenie" style={{marginLeft:"15px"}}>
                            <h2>Ваше фото</h2>
                            <img className="IconImgObyavlenie"src={imgUrl2?imgUrl2:imageVladelecInfo} alt="" />
                            <button onClick={btnChoose2} type="button" className="Voyti choose">Выберите файл</button>
                            <Field id="file-input4" type="file" value={values.imageVladelec} onChange={handleOnChange2} style={{display:"none"}} />
                          </div>
                        </div>
                          ...
                          <div className="OsnovaOpcii" style={{marginBottom:"20px"}}>
                            ...
                            <div className="delenieOpcii">
                              ...
                              <div className="dropdown">
                                <button className="Spisok" id="FilterMestaDop" type="button" onClick={()=>click()}>{mesta!==""?mesta:"Выберите"}</button>
                              ...
                              <div className="checkbox" style={{display:"flex",marginBottom:"10px",marginTop:"30px"}}>
                                <input type="checkbox" value={"Газоваяплита"} id="checkbox_moduleChange1" onChange={() => ClickCheckbox(1)} className="checkBoxOpcii3"/> 
                                <label htmlFor="checkbox_moduleChange1" className="textCheckboxOpcii">Газовая плита</label>
                              </div>
                              <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                <input type="checkbox" value={"Духовка"} id="checkbox_moduleChange2" onChange={() => ClickCheckbox(2)} className="checkBoxOpcii3"/> 
                                <label htmlFor="checkbox_moduleChange2" className="textCheckboxOpcii">Духовка</label>
                              </div>
                              <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                <input type="checkbox" value={"Кофеварка"} id="checkbox_moduleChange3" onChange={() => ClickCheckbox(3)} className="  checkBoxOpcii3"/> 
                                <label htmlFor="checkbox_moduleChange3" className="textCheckboxOpcii">Кофеварка</label>
                              </div>
                              <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                <input type="checkbox" value={"Микроволноваяпечь"} id="checkbox_moduleChange4" onChange={() => ClickCheckbox(4)} className="  checkBoxOpcii3"/> 
                                <label htmlFor="checkbox_moduleChange4" className="textCheckboxOpcii">Микроволновая печь </label>
                              </div>
                              <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                <input type="checkbox" value={"Посуда"} id="checkbox_moduleChange5" onChange={() => ClickCheckbox(5)} className="  checkBoxOpcii3"/> 
                                <label htmlFor="checkbox_moduleChange5" className="textCheckboxOpcii">Посуда </label>
                              </div>
                              <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                <input type="checkbox" value={"Посудомоечнаямашина"} id="checkbox_moduleChange6" onChange={() => ClickCheckbox(6)} className="  checkBoxOpcii3"/> 
                                <label htmlFor="checkbox_moduleChange6" className="textCheckboxOpcii">Посудомоечная машина </label>
                              </div>
                            </div>
                          {/* Второе деление */}
                            <div className="delenieOpcii">
                              ...
                              <div className="dropdown">
                                <button style={{visibility:"hidden"}} className="Spisok" type="button">{}</button>
                                <input type="text" name="select__category" value="" className="drodown__item__hiden" />
                              </div>
                              <div className="checkbox" style={{display:"flex",marginBottom:"10px",marginTop:"30px"}}>
                                <input type="checkbox" value={"Газоваяплита"} id="checkbox_moduleChange7" onChange={() => ClickCheckbox(7)} className="  checkBoxOpcii3"/> 
                                <label htmlFor="checkbox_moduleChange7" className="textCheckboxOpcii">Газовая плита</label>
                              </div>
                              <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                <input type="checkbox" value={"Духовка"} id="checkbox_moduleChange8" onChange={() => ClickCheckbox(8)} className="  checkBoxOpcii3"/> 
                                <label htmlFor="checkbox_moduleChange8" className="textCheckboxOpcii">Духовка</label>
                              </div>
                              <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                <input type="checkbox" value={"Кофеварка"} id="checkbox_moduleChange9" onChange={() => ClickCheckbox(9)} className="  checkBoxOpcii3"/> 
                                <label htmlFor="checkbox_moduleChange9" className="textCheckboxOpcii">Кофеварка</label>
                              </div>
                              <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                <input type="checkbox" value={"Микроволноваяпечь"} id="checkbox_moduleChange10" onChange={() => ClickCheckbox(10)} className=" checkBoxOpcii3"/> 
                                <label htmlFor="checkbox_moduleChange10" className="textCheckboxOpcii">Микроволновая печь </label>
                              </div>
                              <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                <input type="checkbox" value={"Посуда"} id="checkbox_moduleChange11" onChange={() => ClickCheckbox(11)} className="checkBoxOpcii3"/> 
                                <label htmlFor="checkbox_moduleChange11" className="textCheckboxOpcii">Посуда </label>
                              </div>
                              <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                <input type="checkbox" value={"Посудомоечнаямашина"} id="checkbox_moduleChange12" onChange={() => ClickCheckbox(12)} className="  checkBoxOpcii3"/> 
                                <label htmlFor="checkbox_moduleChange12" className="textCheckboxOpcii">Посудомоечная машина </label>
                              </div>
                            </div>
                            {/* Третие делене */}
                            <div className="delenieOpcii">
                              ...
                              <div className="checkbox" style={{display:"flex",marginBottom:"10px",marginTop:"30px"}}>
                                <input type="checkbox" value={"Газоваяплита"} id="checkbox_moduleChange13" onChange={() => ClickCheckbox(13)} className="  checkBoxOpcii3"/> 
                                <label htmlFor="checkbox_moduleChange13" className="textCheckboxOpcii">Газовая плита</label>
                              </div>
                              <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                <input type="checkbox" value={"Духовка"} id="checkbox_moduleChange14" onChange={() => ClickCheckbox(14)} className="  checkBoxOpcii3"/> 
                                <label htmlFor="checkbox_moduleChange14" className="textCheckboxOpcii">Духовка</label>
                              </div>
                              <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                <input type="checkbox" value={"Кофеварка"} id="checkbox_moduleChange15" onChange={() => ClickCheckbox(15)} className="  checkBoxOpcii3"/> 
                                <label htmlFor="checkbox_moduleChange15" className="textCheckboxOpcii">Кофеварка</label>
                              </div>
                              <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                <input type="checkbox" value={"Микроволноваяпечь"} id="checkbox_moduleChange16" onChange={() => ClickCheckbox(16)} className="  checkBoxOpcii3"/> 
                                <label htmlFor="checkbox_moduleChange16" className="textCheckboxOpcii">Микроволновая печь </label>
                              </div>
                              <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                <input type="checkbox" value={"Посуда"} id="checkbox_moduleChange17" onChange={() => ClickCheckbox(17)} className="  checkBoxOpcii3"/> 
                                <label htmlFor="checkbox_moduleChange17" className="textCheckboxOpcii">Посуда </label>
                              </div>
                              <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                <input type="checkbox" value={"Посудомоечнаямашина"} id="checkbox_moduleChange18" onChange={() => ClickCheckbox(18)} className="  checkBoxOpcii3"/> 
                                <label htmlFor="checkbox_moduleChange18" className="textCheckboxOpcii">Посудомоечная машина </label>
                              </div>
                            </div>
                          {/* Четвертое деление */}
                            <div className="delenieOpcii">
                            ...
                            <p className="titleOpcii">Метро</p>
                              ...
                              <div className="checkbox" style={{display:"flex",marginBottom:"10px",marginTop:"30px"}}>
                                  <input type="checkbox" value={"Газоваяплита"} id="checkbox_moduleChange19" onChange={() => ClickCheckbox(19)} className="  checkBoxOpcii3"/> 
                                  <label htmlFor="checkbox_moduleChange19" className="textCheckboxOpcii">Газовая плита</label>
                                </div>
                                <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                  <input type="checkbox" value={"Духовка"} id="checkbox_moduleChange20" onChange={() => ClickCheckbox(20)} className="  checkBoxOpcii3"/> 
                                  <label htmlFor="checkbox_moduleChange20" className="textCheckboxOpcii">Духовка</label>
                                </div>
                                <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                  <input type="checkbox" value={"Кофеварка"} id="checkbox_moduleChange21" onChange={() => ClickCheckbox(21)} className="  checkBoxOpcii3"/> 
                                  <label htmlFor="checkbox_moduleChange21" className="textCheckboxOpcii">Кофеварка</label>
                                </div>
                                <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                  <input type="checkbox" value={"Микроволноваяпечь"} id="checkbox_moduleChange22" onChange={() => ClickCheckbox(22)} className="  checkBoxOpcii3"/> 
                                  <label htmlFor="checkbox_moduleChange22" className="textCheckboxOpcii">Микроволновая печь </label>
                                </div>
                                <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                  <input type="checkbox" value={"Посуда"} id="checkbox_moduleChange23" onChange={() => ClickCheckbox(23)} className="  checkBoxOpcii3"/> 
                                  <label htmlFor="checkbox_moduleChange23" className="textCheckboxOpcii">Посуда </label>
                                </div>
                                <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                  <input type="checkbox" value={"Посудомоечнаямашина"} id="checkbox_moduleChange24" onChange={() => ClickCheckbox(24)} className="  checkBoxOpcii3"/> 
                                  <label htmlFor="checkbox_moduleChange24" className="textCheckboxOpcii">Посудомоечная машина </label>
                                </div>
                            </div>
                          {/* Пятое деление */}
                            <div className="delenieOpcii">
                              ...
                              <div className="checkbox" style={{display:"flex",marginBottom:"10px",marginTop:"30px"}}>
                                  <input type="checkbox" value={"Газоваяплита"} id="checkbox_moduleChange25" onChange={() => ClickCheckbox(25)} className="  checkBoxOpcii3"/> 
                                  <label htmlFor="checkbox_moduleChange25" className="textCheckboxOpcii">Газовая плита</label>
                                </div>
                                <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                  <input type="checkbox" value={"Духовка"} id="checkbox_moduleChange26" onChange={() => ClickCheckbox(26)} className="  checkBoxOpcii3"/> 
                                  <label htmlFor="checkbox_moduleChange26" className="textCheckboxOpcii">Духовка</label>
                                </div>
                                <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                  <input type="checkbox" value={"Кофеварка"} id="checkbox_moduleChange27" onChange={() => ClickCheckbox(27)} className="  checkBoxOpcii3"/> 
                                  <label htmlFor="checkbox_moduleChange27" className="textCheckboxOpcii">Кофеварка</label>
                                </div>
                                <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                  <input type="checkbox" value={"Микроволноваяпечь"} id="checkbox_moduleChange28" onChange={() => ClickCheckbox(28)} className="  checkBoxOpcii3"/> 
                                  <label htmlFor="checkbox_moduleChange28" className="textCheckboxOpcii">Микроволновая печь </label>
                                </div>
                                <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                  <input type="checkbox" value={"Посуда"} id="checkbox_moduleChange29" onChange={() => ClickCheckbox(29)} className="  checkBoxOpcii3"/> 
                                  <label htmlFor="checkbox_moduleChange29" className="textCheckboxOpcii">Посуда </label>
                                </div>
                                <div className="" style={{display:"flex",marginBottom:"10px"}}>
                                  <input type="checkbox" value={"Посудомоечнаямашина"} id="checkbox_moduleChange30" onChange={() => ClickCheckbox(30)} className="  checkBoxOpcii3"/> 
                                  <label htmlFor="checkbox_moduleChange30" className="textCheckboxOpcii">Посудомоечная машина </label>
                                </div>
                            </div>
                          </div>
                        </div>
                        <div className={ToggleState===3?"DopInfo":"DopInfoNone"}>
                          <div className="Message">
                            <p className="kontakti__text">Описание объявления</p>
                              <textarea name="textArea" className={(opisanieInfo=="")?"TextAreaMes__error":"TextAreaMes"} onChange={handleOnChange20} value={opisanieInfo} placeholder="Описание" cols={30} rows={10}></textarea>
                          </div>
                        </div>
                        <div className="" style={{display:"flex",justifyContent:"center"}}>
                            <div style={{width:"fit-content",padding:"9px 16px"}} className={("OshibkaVvoda" && ((cityInfo=="" || kolvoInfo=="" || komnatiInfo=="" || linkViberInfo=="" || linkWatsInfo=="" || linkMailInfo=="" || mailInfo=="" || metroInfo=="" || nameInfo=="" || numberInfo=="" || ploshadInfo=="" || rayonInfo=="" || sentInfo=="" )))? "OshibkaVvoda fix":"OshibkaVvoda"}>
                        ...
                        <div className="" style={{display:"flex",justifyContent:"center"}}>
                          <button className="Voyti choose" disabled={(cityInfo=="" || kolvoInfo=="" || komnatiInfo=="" || linkViberInfo=="" || linkWatsInfo=="" || linkMailInfo=="" || mailInfo=="" || metroInfo=="" || nameInfo=="" || numberInfo=="" || ploshadInfo=="" || rayonInfo=="" || sentInfo=="" || opisanieInfo=="")?isValid:!isValid} type="submit" onClick={()=>{handleSubmit}}>Внести изменения</button>
                        </div>
                      </Form>
                        )}
                    </Formik>
